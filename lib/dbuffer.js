...
DBusBuffer.prototype.unusedFds = function() {
  if (!this.fds || this.fdsIndex >= this.fds.length)
    return;
  return this.fds.splice(this.fdsIndex, fds.length - this.fdsIndex)
}
...
DBusBuffer.prototype.readSimpleType = function readSimpleType(t) {
  ...
  case 'h':
    ...
    if (index >= this.fds.length)
      throw new Error("not enough file descriptors to decode 'h' field");
    this.fdsIndex = Math.max(this.fdsIndex, index + 1);
