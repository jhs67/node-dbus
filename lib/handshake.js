...
var exports = module.exports = function auth(stream, opts, cb) {
  ...
  stream.write('\0');
  tryAuth(stream, authMethods.slice(), opts.fds, cb);
};

function tryAuth(stream, methods, fds, cb) {
  if(methods.length === 0) {
  ...
  function beginOrNextAuth() {
    readLine(stream, function(line) {
      var ok = line.toString('ascii').match(/^([A-Za-z]+) (.*)/);
        if (ok && ok[1] === 'OK') {
          if (fds)
            return negotiateFds(ok[2]);
          stream.write('BEGIN\r\n');
          return cb(null, ok[2]); // ok[2] = guid. Do we need it?
        } else {
          // TODO: parse error!
          if (!methods.empty)
            tryAuth(stream, methods, fds, cb);
          else
            return cb(line);
        }
      });
  ...
  function negotiateFds(oid) {
    stream.write("NEGOTIATE_UNIX_FD\r\n");
    readLine(stream, function(line) {
      var ok = line.toString('ascii').match(/^([A-Za-z_]+)/);
      if (ok && ok[1] === 'AGREE_UNIX_FD') {
        stream.write('BEGIN\r\n');
        return cb(null, oid); // ok[2] = guid. Do we need it?
      } else {
          return cb(line);
      }
    });
